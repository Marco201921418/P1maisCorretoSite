const PerguntaEntrada = document.getElementById('Pergunta');
const resultadoPergunta = document.getElementById('resultadoPergunta');

//função que vai pegar o inpur question

PerguntaEntrada.addEventListener("keypress",(e)=>{
    if(PerguntaEntrada.value && e.key === "Enter")
        sendQuestion();
});
...
function sendQuestion(){
    var sQuestion = PerguntaEntrada.value;
    ...
    .then((json) => {
        if(resultadoPergunta.value) resultadoPergunta.value += "\n";

        if(json.error?.message){
            resultadoPergunta.value += "Erro: " + json.error.message;
        }else if(json.choices?.[0].text){  
            var text = json.choices[0].text || " Sem resposta";
            resultadoPergunta.value += "Chat gpt: " + text;
        }
        resultadoPergunta.scrollTop = resultadoPergunta.scrollHeight;

    })
    .catch((error) => console.error("error", error))
    .finally(()=>{
        PerguntaEntrada.value = "";
        PerguntaEntrada.disabled = true;
        PerguntaEntrada.focus();
    });

    if (resultadoPergunta.value) resultadoPergunta.value += "\n\n\n"; // quebra linha */
    resultadoPergunta.value += `EU: ${sQuestion}`
    PerguntaEntrada.value = "carregando..."
    PerguntaEntrada.disabled = true;
